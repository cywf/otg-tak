...
async function buildCISequence() {
  ...
  for (const f of yamls.slice(0, 4)) {
    ...
    if (!jobNames.length) {
      // fallback: look under 'jobs:' then keys
      const blocks = Array.from(text.matchAll(/^\s{0,}jobs:\s*([\s\S]+?)^\S/mg)).map(m=>m[1]);
      if (blocks[0]) {
        for (const ln of blocks[0].split(/\r?\n/)) {
          const m = /^\s{2}([A-Za-z0-9_-]+):\s*$/.exec(ln);
